...
# 1. 读取数据（保留您的绝对路径）
data_path = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step4_subtype/Step1_INTSlope/Slope_HAMD/subtype1_slope_HAMD.csv'
...
res_hamd = smf.ols('HAMD_0w ~ age + sex + mean_fd', data=df).fit().resid
...
# 设置图表标签和标题
plt.xlabel('HAMD_0w (Adjusted for Covariates)')
plt.ylabel('Slope (Adjusted for Covariates)')
plt.title('Partial Correlation: HAMD_0w vs Slope')
plt.legend(loc='upper right')
plt.tight_layout()

# 4. 保存图片（保留您的绝对路径）
output_path = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step4_subtype/Step1_INTSlope/Slope_HAMD/subtype1_slope_hamd.png'
