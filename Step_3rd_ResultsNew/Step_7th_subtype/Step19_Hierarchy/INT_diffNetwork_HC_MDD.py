...
# 建议为 HC vs Subtype1 创建一个新的输出文件夹以免覆盖原数据
out_dir = os.path.join(base_dir, 'NetworkHierarchy_Subtype2_vs_HC')
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

# 修改：读取 Subtype1 和 HC 的文件
file1 = os.path.join(base_dir, 'subtype2_INT_7net_agesex_FD.csv')
file2 = os.path.join(base_dir, 'HC_INT20_7net_agesex_FD.csv')
...
df2 = pd.read_csv(file2) # 现在 df2 代表 HC 组
...
def flatten_and_merge(df_orig, diff_3d, group_label):
    ...
    df_covariates['Group'] = group_label

    # 拼接并返回
    return pd.concat([df_covariates, df_edges], axis=1)

# 处理两组数据并保存：1 代表 Subtype1，0 代表 HC
...
#df2_flat.to_csv(os.path.join(out_dir, 'HC_edges_flattened.csv'), index=False)
...
# 注意：Group的主效应，Subtype1(1) vs HC(0) 意味着 T为正则 Subtype1 的差值更大
...
t_map_df.to_csv(os.path.join(out_dir, 't_map_matrix_covariates_HC_vs_Sub2.csv'))
p_map_df.to_csv(os.path.join(out_dir, 'p_map_corrected_matrix_covariates_HC_vs_Sub2.csv'))
print("需求2已完成：控制年龄、性别、meanFD的组间比较已完成并保存。")

# ----------------- 5. 绘制 T-map 热图 -----------------
# 创建带 '*' 的文本标注矩阵，p_corr < 0.05 的位置打星号
...
ax = sns.heatmap(t_map_df, cmap='coolwarm', center=0,
                 ...
                 cbar_kws={"label": "T-value (Subtype2 vs HC)"})

# 修改：更新图表标题
plt.title('T-map of Network Differences\n(Subtype2 vs HC, Controlling for Age, Sex, meanFD; * FDR $p<0.05$)')
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
plt.tight_layout()

# 保存热图
heatmap_path = os.path.join(out_dir, 't_map_heatmap_HC_vs_Sub2.png')
