...
# 1. 加载数据
# ==========================================
hc = pd.read_csv("/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/AgeSex/HC_INT20_7net_agesex.csv")
mdd = pd.read_csv("/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/AgeSex/MDD_INT20_7net_agesex.csv")
...
# 3. 年龄差异分析 (匹配前 vs 匹配后)
# ==========================================
t_pre, p_pre = ttest_ind(hc['age'], mdd['age'])
t_post, p_post = ttest_ind(hc_matched['age'], mdd_matched['age'])

print(f"匹配前样本量: HC={len(hc)}, MDD={len(mdd)}")
print(f"匹配后样本量: 每组 {len(hc_matched)} 人")
print(f"匹配前年龄差异: t={t_pre:.4f}, p={p_pre:.4f}")
print(f"匹配后年龄差异: t={t_post:.4f}, p={p_post:.4f}")
...
results = []
for net in networks:
    # 建立 GLM 模型，控制残余的年龄和性别影响
    formula = f"{net} ~ C(Group, Treatment('HC')) + age + sex"
...
plot_df = df_matched.melt(id_vars=["Group"], value_vars=networks,
                          var_name="Network", value_name="INT")
plot_df["Network"] = pd.Categorical(plot_df["Network"], categories=networks, ordered=True)

plt.figure(figsize=(16, 8))
# 箱线图
ax = sns.boxplot(data=plot_df, x="Network", y="INT", hue="Group",
                 showfliers=False, palette="Set2")
# 散点图
sns.stripplot(data=plot_df, x="Network", y="INT", hue="Group",
...
y_max_vals = plot_df.groupby("Network")["INT"].max()
for i, net in enumerate(networks):
    p_fdr = res_df.loc[res_df["Network"] == net, "p_FDR"].values[0]
    star = p_to_star(p_fdr)
    if star:
        y_pos = y_max_vals[net] + (plot_df["INT"].max() - plot_df["INT"].min()) * 0.05
        ax.text(i, y_pos, star, ha='center', color='red', fontsize=16, fontweight='bold')

# plt.title(f"INT Differences After Age/Sex Matching (N={len(hc_matched)} per group)\nAge Post-match p={p_post:.4f}",
#           fontsize=16)
plt.xticks(rotation=45)
plt.tight_layout()

# 保存结果
plt.savefig("/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/AgeSex/HCMDD_matched_network_diff.png", dpi=300)
res_df.to_csv("/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/AgeSex/HCMDD_matched_glm_results.csv", index=False)