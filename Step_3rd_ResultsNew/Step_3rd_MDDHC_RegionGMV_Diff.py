...
# 1. 加载数据
HCData = pd.read_csv('/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step3_Structure/HC_GrayVol246.csv')
MDDData = pd.read_csv('/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step3_Structure/MDD_GrayVol246.csv')
...
# 3. 提取脑区列名（自动排除 subID, TIV, age, sex, Group 等非脑区特征列）
# ***修改点 1: 在列表中加入 'TIV'***
...
print("开始进行控制年龄、性别和 TIV 的 246 脑区差异分析...")

# 4. 循环 246 个脑区，使用 OLS 协方差分析替代独立的 T 检验
for i, region in enumerate(brainRegion):

    # ***修改点 2: 公式中加入 TIV，并明确 HC 为参照组***
    # 使用 Q() 函数包裹脑区名称，防止如 "46d_L" 这种以数字开头的列名导致公式解析报错
    formula = f'Q("{region}") ~ C(Group, Treatment(reference="HC")) + age + C(sex) + TIV'
...
csv_path = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step3_Structure/Ttest_Region_HCMDDGLM.csv'
...
output_nii = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step3_Structure/Ttest_Region_HCMDDGLM.dscalar.nii'
