...
from sklearn.model_selection import KFold, GridSearchCV  # 导入网格搜索用于自动调参
from sklearn.metrics import r2_score

# 1. 读取数据
file_name = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step5_GMVpredictedINT/MDD_BN246GMV_8NetINT.csv'
...
for i, target_name in enumerate(target_cols):
    ...
    for train_idx, test_idx in kf.split(X):
        ...
        # 【核心修改点】
        # 由于 SVR 对超参数（C 和 gamma）较为敏感，使用 GridSearchCV 自动优化选择最佳参数（防过拟合）
        # cv=5 表示内部进行5折交叉验证来选参数，n_jobs=-1 表示使用所有CPU核心加速计算
        param_grid = {
            'C': [0.1, 1, 10, 100],
            'gamma': ['scale', 'auto']
        }
        model = GridSearchCV(SVR(kernel='rbf'), param_grid, cv=5, n_jobs=-1)
...
# 仅保存 8 个网络的 R2 评分以及它们的平均 R2（1行指标数据）
r2_data = {'subID': ['R2']}
for i, target_name in enumerate(target_cols):
...
# 建议将输出文件名改为 svr_r2_results.csv 以防覆盖之前的 Ridge 结果，此处仍按原逻辑路径保存
output_path = '/Volumes/QC/INT/INT_BN246_HC135BP_MDD135BP_DZIII/Step5_GMVpredictedINT/svr_r2_results.csv'
df_r2_only.to_csv(output_path, index=False)
print(f"[成功] 指标汇总结果已保存至 '{output_path}'")