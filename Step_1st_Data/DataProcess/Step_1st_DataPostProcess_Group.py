...
path = "/Volumes/ZLabData/BrainProject/brainproject_II/fmriprep/MDD_DZ_1/sub-*V01/func/sub-*V01_task-rest_space-fsLR_den-91k_bold.dtseries.nii"
data = glob.glob(path)

for i in data:
    subID = i.split("/")[-3]
    print(subID)
    if subID in ['sub-07000043V01']:
        continue
    # ====================== 1. 加载 Confounds & 获取 Mask ======================
    # 使用绝对路径以确保 wb_command 运行安全
    out_file = os.path.abspath("/Volumes/QC/Data/INT/DZ/MDD/"+subID+"_task-rest_acq-ap_run-1_space-fsLR_den-91k_bold_cleaned.dtseries.nii")
    smoothed_file = os.path.abspath("/Volumes/QC/Data/INT/DZ/MDD/"+subID+"_task-rest_acq-ap_run-1_space-fsLR_den-91k_bold_smooth.dtseries.nii")

    if os.path.isfile(smoothed_file):
        continue
